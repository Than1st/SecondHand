...
class BottomSheetEditCategory(
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        editViewModel.getCategoryHome()
        editViewModel.category.observe(viewLifecycleOwner){
            when(it.status){
                Status.SUCCESS -> {
                    progressDialog.dismiss()
                    if(it.data != null){
